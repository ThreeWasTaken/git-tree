...
def commit_hash(ref: str, color: str | None = None) -> str:
    output = run_git(["show", "-s", "--format=%h", ref], exit_on_error=False).strip()
    value = output or ref
    return f"{color}{value}{RESET}" if color else value


def commit_subject(ref: str, color: str | None = None) -> str:
    subject = run_git(["show", "-s", "--format=%s", ref], exit_on_error=False).strip()
    return f"{color}{subject}{RESET}" if color and subject else subject


def raw_commit_author(ref: str) -> tuple[str, str] | None:
    output = run_git(["show", "-s", "--format=%an%x09%ar", ref], exit_on_error=False).strip()
    if not output:
        return None
    try:
...
def commit_author(ref: str) -> str:
    ...
    return format_author(author, relative_date, load_author_config())


def commit_summary(ref: str, color: str | None = None) -> str:
    output = run_git(["show", "-s", "--format=%h %s", ref], exit_on_error=False).strip()
    summary = output or ref
    return f"{color}{summary}{RESET}" if color else summary
...
def get_author_context(target: str, staged: bool, all_mode: bool) -> str | None:
    ...
    if ".." in target:
        ...
        return (
            f"{MAGENTA}Authors:{RESET} "
            f"{CYAN}{left_author}{RESET} "
            f"{MAGENTA}→{RESET} "
            f"{CYAN}{right_author}{RESET}"
        )
    ...
    return f"{MAGENTA}Author:{RESET} {CYAN}{author}{RESET}"
...
def get_rebase_context() -> str | None:
    ...
    if os.path.isdir(rebase_merge):
        ...
        return f"Rebase: applying {commit_summary(sha, YELLOW)}" if sha else "Rebase: in progress"
...
def get_conflict_files() -> set[str]:
    output = run_git(["diff", "--name-only", "--diff-filter=U"], exit_on_error=False)
